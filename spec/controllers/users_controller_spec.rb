		...
		context 'avatar' do
			...
			context 'create' do
				...
				it 'should 400 for particial user avatar data' do
					post :create_avatar, {id: @correct_user.id, data: @file_content}
					response.status.should eq 400

					post :create_avatar, {id: @correct_user.id, data: @file_content, content_type: 'js'}
					response.status.should eq 400

					post :create_avatar, {id: @correct_user.id, data: @file_content, content_type: 'js', crop_x: 0}
					response.status.should eq 400

					post :create_avatar, {id: @correct_user.id, data: @file_content, content_type: 'js', crop_x: 0, crop_y: 0}
					response.status.should eq 400
				end

				it 'should 400 for illegal avatar content type' do
					post :create_avatar, {id: @correct_user.id, data: @file_content, content_type: 'js', crop_x: 0, crop_y: 0, crop_l: 0}
					response.status.should eq 400
				end
			...
			context 'read update delete' do
				...
				it 'get all should 404 for not existed user' do
					get :get_avatars, @id_wrong_structure
					response.status.should eq 404
				end

				it 'get all should 200 & correct data' do
					get :get_avatars, @id_structure
					response.status.should eq 200
				...
				it 'set current should 404 for not existed user' do
					...
					get :set_current_avatar, @id_wrong_structure.merge({avatar_id: (@ids[0] - 1)})
					response.status.should eq 404
				end

				it 'set current should 404 for not existed avatar' do
					cookies['ACCESS_TOKEN'] = @id_structure
					get :set_current_avatar, @id_structure.merge({avatar_id: (@ids[0] - 1)})
					response.status.should eq 404
				end

				it 'set current should 403 for not owner' do
					cookies['ACCESS_TOKEN'] = @new_user.access_token
					get :set_current_avatar, @id_structure.merge({avatar_id: @ids[0]})
					response.status.should eq 403
				end

				it 'set current should 200 for correct params' do
					@ids.each do |id|
						get :set_current_avatar, @id_structure.merge({avatar_id: id})
						response.status.should eq 200
				...
				it 'set double current should 200' do
					get :set_current_avatar, @id_structure.merge({avatar_id: @current.id})
					response.status.should eq 200
				...
				it 'delete should 200 for correct params' do
					@ids.each do |id|
						...
						delete :delete_avatar, @id_structure.merge({avatar_id: id})
						response.status.should eq 200
